...
public class SmsUtil {
    ...
    public static boolean sendMessage(String phoneNumbers, SmsSendDTO smsSendDTO) {
        ...
        try {
            ...
            if (code.equals(SmsConstant.SUCCESS_CODE)) {
                ...
                log.error("短信发送失败，手机号：{},模板：{},参数：{},原因：{}", phoneNumbers, templateCode, json,
     ...
     * @since 2025/4/17 11:03
     */
    public static List<QuerySmsTemplateListResponseBody.QuerySmsTemplateListResponseBodySmsTemplateList>
    ...
    public static String getTemplateContent(String templateCode) {
        List<QuerySmsTemplateListResponseBody.QuerySmsTemplateListResponseBodySmsTemplateList> list =
            querySmsTemplateList();
        if (CollUtil.isEmpty(list)) {
            return "";
        }
        String templateContent = list.stream().filter(item -> item.getTemplateCode().equals(templateCode)).findFirst()
            .map(QuerySmsTemplateListResponseBody.QuerySmsTemplateListResponseBodySmsTemplateList::getTemplateContent)
            .orElse("");
